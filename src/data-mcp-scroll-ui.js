...
function boot(){
  ...
  if(wrap)wrap.style.display=tab==='test'?'':'none';
  if(shell){shell.style.display=tab==='test'?'none':'';shell.classList.toggle('data-shell-mcp-scroll',tab==='mcp'||!!shell.querySelector('.mcp-session-card'))}
}
boot();
